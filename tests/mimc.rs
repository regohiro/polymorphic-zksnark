...
fn test_mimc() {
    ...
    let mut proof_vec: Vec<Proof<Bls12_377>> = vec![];
    ...
    for _ in 0..SAMPLES {
        ...
        let proof = Groth16::<Bls12_377>::prove(&pk, c, &rnd).unwrap();

        println!("proof.a: {:?}", proof.a);

        proof_vec.push(proof.clone());
        ...
        assert!(
            Groth16::<Bls12_377>::verify_with_processed_vk(&pvk, &[image], &proof, &rnd).unwrap()
        );
    ...
    assert!(Groth16::<Bls12_377>::verify_all_proofs(&proof_vec).unwrap());
